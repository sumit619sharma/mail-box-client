...
const Home = () => {
      ...
      const fromMail = localStorage.getItem('email').replace(/[@.]/g, '');
            try {
              const resp=  await fetch(`https://react-http-2f680-default-rtdb.firebaseio.com/mails/${fromMail}.json`,{
                method:'POST',
           ...
           const sendMailTo =async (item)=>{
            ...
            try {
              const resp=  await fetch(`https://react-http-2f680-default-rtdb.firebaseio.com/mails/${toMail}.json`,{
                method:'POST',
                body: JSON.stringify(item),
                headers:{
                  'Content-Type': 'application/json'
